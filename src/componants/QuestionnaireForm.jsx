...
const QuestionnaireForm = () => {
  ...
  const handleInitialSubmit = async (e) => {
    ...
    try {
      ...
      if (JSON.parse(response.data)?.["Clarifying Questions"]) {
        ...
        const summaryResponse = await submitToChat(data);
  ...
  const submitToChat = async (data) => {
    try {
      const res = await axios.post(
        `https://9892-35-196-55-140.ngrok-free.app/diagnose/diagnosis`,
        {
          patient_narrative: data,
        }
      );
      return JSON.parse(res.data)["Summary"];
    } catch (error) {
  ...
  const handleFollowUpSubmit = async (e) => {
    ...
    if (currentQuestionIndex < followUpQuestions.length - 1) {
      ...
      try {
        const summaryResponse = await submitToChat(JSON.stringify(allAnswers));
        setSummary(summaryResponse);
  ...
  const SummaryDisplay = () => (
    ...
    >
      <div className="flex items-center justify-center gap-2 text-green-600 mb-8">
        <CheckCircle size={24} />
        <span className="text-xl font-medium">Assessment Complete</span>
      </div>

      <div className="bg-blue-50 p-6 rounded-xl">
        <h2 className="text-xl font-semibold mb-4 text-blue-800">Summary</h2>
        <div className="prose prose-blue max-w-none">
          {summary?.split("\n").map((paragraph, index) => (
            <p key={index} className="text-blue-700 mb-4">
              {paragraph}
            </p>
          ))}
        </div>
      ...
      <div className="flex justify-center gap-4 mt-8">
        <motion.button
          ...
          className="px-8 py-3 bg-blue-600 text-white rounded-full font-medium shadow-xl hover:shadow-2xl transition-all"
        >
          Continue to Chat
        </motion.button>
        <motion.button
