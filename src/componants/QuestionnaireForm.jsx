...
//       if (response.data?.["Clarifying Questions"]) {
//         setFollowUpQuestions(response.data["Clarifying Questions"]);
...
//           patient_narrative: JSON.stringify(allAnswers),
//         });
//         navigate("/chat");
//       } catch (error) {
...
const QuestionnaireForm = () => {
    ...
    try {
      ...
      if (response.data?.["Clarifying Questions"]) {
        const clarifyingQuestions = Array.isArray(
          response.data["Clarifying Questions"]
        )
          ? response.data["Clarifying Questions"]
          : [response.data["Clarifying Questions"]];
  ...
  const submitToChat = async (data) => {
    try {
      await axios.post(`https://2085-35-226-51-201.ngrok-free.app/questions`, {
        patient_narrative: data,
      });
    } catch (error) {
